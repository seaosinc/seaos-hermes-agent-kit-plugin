...
アプリの名前・ボットの名前（メンションに使う名前）・説明は変えられます。どの項目が Slack 上のどこに出るか、
使える文字の決まりは、README のマニフェストの上の表にあります。
...
1. 使いたいチャンネルに App を招待する（チャンネルで `/invite @seaos`。ボットの名前を変えた場合はその名前）
2. **ボットの名前でメンションして**（マニフェストのままなら `@seaos`）話しかける。チャンネルでは、メンションしないと返事をしません
3. 返事はスレッドに返ってきます
