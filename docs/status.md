...
0. **Hermes 本体のパッチが当たったままである。**（最優先で確認する）

   2026-09-10 の版で、ディスク上のデスクトッププラグインが読み込めなくなった。
   `dist/assets/sdk-*.js` の `Rg` を getter にする1行を手で当てて動かしている。
   **`hermes update` で消える。** 消えると SEAOS の画面が出なくなる。

   - 上流で直っていれば、パッチは不要
   - 直っていなければ issue を出す（原因は特定済み: バンドル内で SDK の実体が
     使う側より後ろで定義され、`undefined` を掴む）
   - 当て直す場合、アセット名（ハッシュ）が変わっているので作り直しが要る

   **これは「Hermes 本体は改造しない」という原則を破っている状態**なので、
   長く置かない。
