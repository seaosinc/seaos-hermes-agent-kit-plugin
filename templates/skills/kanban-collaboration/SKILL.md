...
ツール（`kanban_create`）で 1800 を下回る値を渡すと、プラグイン `runtime-floor` が作る前に
1800 へ直す。**CLI の `--max-runtime` はそれを通らない**ので、毎分走る `runtime-guard` が
あとから直すが、間に合わずに死ぬことがある。CLI で作るときは値を付けないか、1800 以上を付ける。
...
**ホストにいる役は、そのパスをそのまま読む。** 作業部屋の中にいる役は
`seaos-path "<パス>"` を通してから読む（macOS では同じパスが返り、Windows では部屋の中の
パスに読み替わる）。部屋の中では読み取り専用なので、加工するなら `/workspace` へ写してから。
