...
5. 起動したら、**画面左下のゲートウェイの表示から「再起動」を押す**

> **4 と 5 の再起動は、初めて入れたときに必ず行ってください。** プラグインは、Hermes やゲートウェイが
> 起動するときに読み込まれます。再起動しないまま SEAOS を開くと、「画面の準備ができていません」と表示されます（そのときは少し待って「再読み込み」、変わらなければ再起動）。
