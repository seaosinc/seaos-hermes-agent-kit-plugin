...
def apply_roles(
    ...
    *,
    ...
    """**あるべき状態に合わせる。** 入っていないものは入れ、外したものは消す。

    OFF は `hermes profile delete` である——セッションと記憶ごと消えて戻せない。
    だから2つの歯止めを置く:

      - **走行中のカードを抱えた役は消さない。** 途中の作業が黙って落ちる
      - **配置表に無い役は触らない。** 知らないものを消さない
    """
    import worker
    ...
    for name in roles.names():
        ...
        if want and not installed:
            code, _ = hermes.install(dist)
            result.lines.append(f"+ {name} を導入" if code == 0 else f"✗ {name} の導入に失敗")
            result.failures += 0 if code == 0 else 1
        elif want and installed:
            if not hermes.is_distribution(name):
                code, _ = hermes.install(dist, force=True)
            else:
                code, _ = hermes.update(name, force_config=force_config)
            ...
            busy = worker.busy_cards(name)
            if busy > 0:
                result.lines.append(f"! {name} は走行中のカードを {busy} 件抱えているので消さない")
                continue
            hermes.run(["profile", "delete", name])
            if profile_dir(name).is_dir():
                result.failures += 1
                result.lines.append(f"✗ {name} を消せなかった")
            else:
                result.lines.append(f"- {name} を削除（セッションと記憶も消えた）")

    if desired & known:
        described = sync_descriptions()
        result.lines.extend(described.lines)
        result.failures += described.failures
        applied = apply_env()
        result.lines.extend(applied.lines)
